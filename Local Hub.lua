if game.CoreGui:FindFirstChild("TurkishHub") then
    TurkishHub_RemoveGui()
end
...
local success, response = pcall(function()
    for key, value in pairs(GameTable) do
        if game.PlaceId == tonumber(key) then
            GameFound = true
            ScriptData = SendRequest("https://raw.githubusercontent.com/ErrorTeam011/DataHub/source/"..value..".lua")
        end
    end
    if not GameFound then
        ScriptData = SendRequest("https://raw.githubusercontent.com/ErrorTeam011/DataHub/source/DataHubUniversal.lua")
    end
